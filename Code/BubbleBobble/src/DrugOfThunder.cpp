...
DOT::DOT(const Point& p, Directions d) : Entity(p, BUBBLE_PHYSICAL_SIZE, BUBBLE_PHYSICAL_SIZE, BUBBLE_FRAME_SIZE, BUBBLE_FRAME_SIZE)
{
	particles = false;
	dire = d;
	speed = .3;
...
void DOT::SetAnimation(int id)
{
	Sprite* sprite = dynamic_cast<Sprite*>(render);
	sprite->SetAnimation(id);
}
AppStatus DOT::Initialise()
{
...
void DOT::SetPlayer(Player* p)
{
	player = p;
}
void DOT::SetPlayer2(Player2* p)
{
	player2 = p;
}
void DOT::Stomp()
...
Point DOT::GetPos() const
{
	return pos;
}
void DOT::EnemyCatchSlime()
{
	if (poped == false)
	{
	switch (bubbleStages)
...
void DOT::SetTileMap(TileMap* m) 
{
	map = m;
}
void DOT::Movement(Directions d)
{
	ClampPos();
	Stomp();
	if (poped == true)
...
void DOT::MoveBubbleRightPlayer()
{
...
void DOT::HandleCollisionLogic()
{
	
}
void DOT::StayBehind(DOT* bub)
{
	pos.x = bub->pos.x - 16;
	
}
void DOT::MoveBubbleToRandomNear()
{
...
void DOT::DrawDebug(const Color & col) const
{
...
Directions DOT::BublePop(Directions d)
{
